"""
Process Surf_Ice colormaps with luminance correction.

This script:
1. Reads all .clut colormap files from Surf_Ice Resources/lut directory
2. Applies map_luminance correction
3. Saves corrected versions to Surf_Ice folder
...
def process_surfice_colormaps(input_dir: str, output_dir: str):
    """
    Apply map_luminance correction to all Surf_Ice colormaps.
    
    Parameters
    ----------
    input_dir : str
        Directory containing Surf_Ice .clut files
    output_dir : str
    ...
    print(f"\nProcessing Surf_Ice colormaps...")
    print(f"  Input: {input_dir}")
    print(f"  Output: {output_dir}")
    
    # Apply map_luminance (this will process all .clut files in the directory)
...
def plot_comparison(before_dir: str, after_dir: str, 
                   output_file: str = 'surfice_colormaps_comparison.png',
                   max_plots: int = None):
    """
    Plot before/after comparison of Surf_Ice colormaps.
    ...
    # Get all .clut files from the output directory
    after_files_paths = sorted(after_path.glob('*.clut'))
    ...
    if not before_files or not after_files:
        print("Warning: No matching .clut files found for comparison")
        return
...
def main():
    ...
    print("="*60)
    print("Processing Surf_Ice Colormaps")
    print("="*60)
    
    # Define directories
    input_dir = '/users/cyrilpernet/bin/Surf_Ice/Resources/lut'
    output_dir = '/indirect/staff/cyrilpernet/brain_colours/Surf_Ice'
    ...
    # Count input files (.clut files)
    input_files = list(Path(input_dir).glob('*.clut'))
    print(f"\n      Found {len(input_files)} colormap files")
    
    # Step 1: Apply luminance correction
    print("\n[1/2] Applying luminance correction...")
    process_surfice_colormaps(input_dir, output_dir)
    
    # Count corrected files
    corrected_files = list(Path(output_dir).glob('*.clut'))
    ...
    print(f"Comparison plot: surfice_colormaps_comparison.png")
    print()
